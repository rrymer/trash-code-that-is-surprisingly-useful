...
def well_intensities_toarray( well, plateID, outname ):
   clusters = well_cluster_peaks( well, None )
   specs = \
        ['negative_peaks', 'negative_peaks', 0, 0], \
        ['negative_peaks', 'positive_peaks', 0, 1], \
   ...
   for s in specs:
       ...
       for i in list(range(x.size)):
           arytmp = [x[i], y[i]]
           print arytmp
           intensityAry = np.concatenate((intensityAry, arytmp), axis = 0)
   print intensityAry
   intensityAry.tofile(arrayFile, sep=",", format="%d")
   return intensityAry
...
#        well = src_mp.get_plate(filelist[0]).wells()["A01"]
    global intFile
    global plateFile
    intFile = outname + ".intensities.csv"
    wellFile = outname + ".wells.csv"
    plateFile = outname + ".plates.csv"
    intF = open(intFile, "w")
    ...
    for j, lwPlate in src_mp.plates.iteritems():
        ...
        for k, well in lwPlate.wells().iteritems():
            if "f" in label:
                    ...
                    well_intensities_toarray( well, j, outname )
...
try:
    ...
    quit()
else: files = glob.iglob(args.src_path)
