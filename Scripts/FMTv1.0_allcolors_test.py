...
class MultiPlate(object):
    ...
    def __str__( self ):
        str=""
        for platename, lwPlate in self.plates.iteritems():
             str += platename + '\n'
             for platename, well in lwPlate.wells().iteritems():
                 str += ','.join( well.name ) +'\n'
        return str
...
def well_intensities_to_str( well, platename ):
    ...
    for s in specs:
        ...
        for i in list(range(x.size)):
            tmp = "%s, %s, %d, %d, %d, %d" % ( platename, well.name, x[i], y[i], s[2], s[3] )
...
#        well = src_mp.get_plate(filelist[0]).wells()["A01"]
    ...
    writer = csv.DictWriter(intF, fieldnames = ["plate", "wells", "VIC.Intensity", "FAM.Intensity", "FAM.Cluster", "VIC.Cluster", "FAM.Width", "VIC.Width"], delimiter = ',')
    writer.writeheader()
